...
| `cat-postgres-project`, `catalogue-*-files` (`COMPOSE_PROJECT_NAME`), `host-print-app`, `chain-print-two` | `print-COMPOSE_PROJECT_NAME STACK=x` names the project after `x` (`tester-postgres-local`), `print-APP` prints the stack name | in make, `print` looked at the myos checkout itself (`tester-myos-local`, `APP myos`) while `up` used the stack |
| `host-print-env-vars`, `cat-drone-env-vars` | `ENV_VARS` holds only the names the compose files of the stack reference | make included every `.mk` of every stack, so `DRONE_SERVER_HOST` leaked into every stack |
...
| `catalogue-*-files` | no `cat: ...: Is a directory` noise | make read `$(CONFIG)/$(ENV)/$(APP)/.env` where APP was a directory |
