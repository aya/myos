...
| `demo-env-update`, `demo-up-renders-env` | `env-update` renders the `.env.dist` of the stacks given (make only read `$WORKDIR/.env.dist`); `up` renders it the first time and reports `env-update ok (N keys added)` | one place for the developer's defaults: the stack directory |
| `host-bootstrap`, `host-install` | `bootstrap` = `.env` rendered, networks, images built; no `usermod`, `binfmt`, `nfsd`, `sysctl`, `ufw` | system setup is not the job of a stack tool; `doctor` reports what is missing |
| `demo-docker-build*`, `demo-build-all`, `host-build`, `chain-build-up-logs` | `build [image[:variant]]` runs `docker build` with three build args (`DOCKER_BUILD_DIR`, `UID`, `GID`) and no labels; a stack without images runs `compose build` plain; the repository is `<user>/<stack>/<env>` | the twenty build args and labels of make leaked the git state, the ssh hosts and the credentials of the remote |
